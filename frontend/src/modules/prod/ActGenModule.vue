...
const actTypes = ['приёмка', 'скрытые работы', 'испытания', 'освидетельствование', 'входной контроль']

async function submit() {
  result.value = null
  start([
    { label: 'Подготовка данных', at: 25 },
    { label: 'Генерация документа', at: 65 },
    { label: 'Финализация', at: 95 },
  ], 8000)
  try {
    result.value = await post('/api/generate-act', { ...form })
...
<template>
  <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    <header class="mb-10 animate-fade-up">
      ...
      <h1 class="font-display text-3xl sm:text-4xl text-carbon-900 mb-3">Акты</h1>
      <p class="text-carbon-500 font-body">{{ store.cardDescriptions.actgen }}</p>
    </header>
    ...
    <section class="bg-white rounded-2xl border border-carbon-200/60 p-8 mb-8 animate-fade-up" style="animation-delay:80ms">
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-6">
        <div>
          <label class="block text-xs font-medium text-carbon-500 mb-1.5">Тип акта</label>
          <select v-model="form.act_type"
            ...
            <option v-for="t in actTypes" :key="t" :value="t">{{ t }}</option>
          </select>
        </div>
        <div>
        ...
        <div>
          <label class="block text-xs font-medium text-carbon-500 mb-1.5">Подрядчик</label>
          <input v-model="form.contractor" type="text"
                 class="w-full px-3 py-2.5 bg-canvas border border-carbon-200/60 rounded-lg text-sm
                        text-carbon-700 focus:outline-none focus:border-sage/50 focus:ring-1
                        focus:ring-sage/20 transition-colors" />
        </div>
      ...
      <div class="mb-6">
        <label class="block text-xs font-medium text-carbon-500 mb-1.5">Выполненные работы</label>
        <textarea v-model="form.works" rows="3"
      ...
      <button @click="submit" :disabled="loading"
        ...
        <span v-else>Сгенерировать акт</span>
      </button>
    </section>
    ...
    <section v-if="result" class="bg-white rounded-2xl border border-carbon-200/60 p-8 animate-scale-in">
      <h2 class="font-display text-xl text-carbon-800 mb-6">Сгенерированный акт</h2>

      <div v-if="result.text || result.content" class="bg-canvas rounded-xl p-6 text-sm text-carbon-700 leading-relaxed whitespace-pre-line font-body">
        {{ result.text || result.content }}
      </div>

      <pre v-if="!result.text && !result.content"
