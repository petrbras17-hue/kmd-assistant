...
async function submit() {
  ...
  start([
    ...
    { label: 'Извлечение текста', at: 40 },
    { label: 'Посимвольное сравнение', at: 70 },
    { label: 'Генерация diff', at: 95 },
  ], 15000)
  try {
    result.value = await post('/api/compare-pdf', null, {
...
<template>
  <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    <header class="mb-10 animate-fade-up">
      ...
      <h1 class="font-display text-3xl sm:text-4xl text-carbon-900 mb-3">Сравнение PDF</h1>
      <p class="text-carbon-500 font-body">{{ store.cardDescriptions.comparepdf }}</p>
    </header>
    ...
    <section class="bg-white rounded-2xl border border-carbon-200/60 p-8 mb-8 animate-fade-up" style="animation-delay:80ms">
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
        <div>
          <label class="block text-sm font-medium text-carbon-600 mb-2">PDF 1 (старая редакция)</label>
        ...
        <div>
          <label class="block text-sm font-medium text-carbon-600 mb-2">PDF 2 (новая редакция)</label>
    ...
    <section v-if="result" class="bg-white rounded-2xl border border-carbon-200/60 p-8 animate-scale-in">
      <h2 class="font-display text-xl text-carbon-800 mb-6">Различия между редакциями</h2>

      <div v-if="result.changes?.length" class="space-y-3">
        <div v-for="(ch, i) in result.changes" :key="i"
             class="bg-canvas rounded-xl p-4">
          <div class="flex items-center gap-2 mb-2">
            <span class="text-xs font-mono px-2 py-0.5 rounded"
                  :class="ch.type === 'added' ? 'bg-status-ok/10 text-status-ok' :
                          ch.type === 'removed' ? 'bg-status-fail/10 text-status-fail' :
                          'bg-status-warn/10 text-status-warn'">
              {{ ch.type === 'added' ? 'Добавлено' : ch.type === 'removed' ? 'Удалено' : 'Изменено' }}
            </span>
            <span v-if="ch.page" class="text-xs font-mono text-carbon-400">стр. {{ ch.page }}</span>
          </div>
          <p class="text-sm text-carbon-600">{{ ch.text || ch.content }}</p>
        </div>
      </div>

      <div v-if="result.similarity !== undefined" class="mt-6 bg-canvas rounded-xl p-5 flex items-center gap-4">
        <span class="text-3xl font-display text-carbon-800">{{ result.similarity }}%</span>
        <span class="text-sm text-carbon-500">Общее сходство документов</span>
      </div>

      <pre v-if="!result.changes && result.similarity === undefined"
